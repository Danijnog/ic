...
def k_means_clustering(high_dim_embeddings, n_clusters) -> KMeans:
    """
    Applies K-Means Clustering to high_dim_embeddings.
    """
    kmeans_model = KMeans(n_clusters=n_clusters)
    cluster_labels = kmeans_model.fit_predict(high_dim_embeddings)

    return cluster_labels


def hdbscan_clustering(high_dim_embeddings, min_cluster_size) -> HDBSCAN:
    """
    Applies HDBSCAN Clustering to high_dim_embeddings.
    """
    hdbscan_model = HDBSCAN(min_cluster_size)
    cluster_labels = hdbscan_model.fit_predict(high_dim_embeddings)
