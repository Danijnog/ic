...
def embedding_scatter_plot(df) -> tuple[px.scatter, pd.DataFrame]:
    ...
    # Check if there are 2D or 3D embeddings on df
    if "z" in df.columns:
        fig = px.scatter_3d(
            df,
            x="x",
...
def get_df_for_plot(low_dim_embeddings, labels, date_labels, clusters_labels) -> pd.DataFrame:
    ...
    # Check if low_dim_embeddings is 2D or 3D
    df = (
        pd.DataFrame(low_dim_embeddings, columns=["x", "y", "z"])
        if low_dim_embeddings.shape[1] == 3
        else pd.DataFrame(low_dim_embeddings, columns=["x", "y"])
    )
...
def get_new_high_dim_embedding(
    high_dim_embeddings, labels, date_labels, clusters_labels, cluster_list, group_list, day_list
) -> pd.DataFrame:
    """
...
def get_df_for_remove_days_groups_that_has_less_25_messages(
