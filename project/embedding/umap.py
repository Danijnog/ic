...
def umap_reduce_dim(embeddings, n_dimensions) -> np.ndarray:
    ...
    """
    umap = UMAP(
        n_components=n_dimensions, spread=1.8, min_dist=0.05, n_neighbors=40, metric="cosine"
    )
    ...
    print(f"Embedding dimension after UMAP: {np.array(low_dim_embeddings).ndim}")
